...
import { createAccountActions } from '@/actions/user'
...
const CreateAccountPage = () => {
  ...
  const handleClickCreateAccountButton = async (formData: FormData) => {
    startTransition(async () => {
      const {errorMessage} = await createAccountActions(formData)
      if (!errorMessage) {
        router.replace("/")
  ...
  <main className="flex min-h-screen items-center justify-center px-4 pb-24">
    <div className="bg-popover relative felx w-full max-w-sm flex-col items-center rounded-lg border p-8">
    <h1
      ...
      <form className={`flex w-full flex-col gap-4 ${isPending && "-z-10 opacity-0"}`}
        ...
        <p className='mt-3 text-center text-xs'>
          ...
          <Link 
          href="/login"
          className="hover:text-primary ml-2 transition-colors duration-200 ease-in-out"
          >
            Login
          </Link> 
        </p>
